...
select_indicex = find(lonp_grid(:,1)>-21.2 & lonp_grid(:,1)<-14.8);
select_indicey = find(latp_grid(1,:)>12.2 & latp_grid(1,:)<18.9);
ba = ba_bc(select_indicex,select_indicey,:,:);
% M2_ba = ba(:,:,1,4);

bg = bg_bc(select_indicex,select_indicey,:,:);
mask = weto_p(select_indicex,select_indicey,:);
btm = bottom_index(select_indicex,select_indicey);
dz = ddpo_grid(select_indicex,select_indicey,:);
lon = lonp_grid(select_indicex,select_indicey);
lat = latp_grid(select_indicex,select_indicey);
ie = size(mask,1);
je = size(mask,2);
...
for i = 1:ie
    for j = 1:je
        if mask(i,j,1)~=0
            ...
            buoyancy_time = ...
                t_predic(Some_time,TIDE(itide),1/TidePeriod(itide),TIDECON(1:btm(i,j),:),lat(i,j));
...
save BC_tide_SSHA_Mss.mat lon lat SSHA_tide
% 
% LONLIMS = [-21 -15];
% LATLIMS = [14.2 18.6];
% 
...
% m_grid('linestyle','none','linewidth',1.5,'fontsize',16,'fontname','Arial',...
%     'xtick',8,'tickdir','in');hold on;
% m_pcolor(lon,lat,SSHA_tide);shading interp;
% clim([-2 2])
% load('BlueWhiteOrangeRed.mat');colormap(BlueWhiteOrangeRed)