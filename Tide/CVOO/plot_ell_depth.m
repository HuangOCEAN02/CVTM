function w = plot_ell_depth(SEMA, ECC, INC, PHA, depth,Eye)
...
%% 
   ...
   % maxmimum
     plot([0 real(wmax)], [0 imag(wmax)], 'color',[.4 .2 .9],'linewidth',2,'LineStyle',':');
     hold on;
    plot3([real(wmax)], [imag(wmax)],[depth], 'color',[.4 .2 .9],'linewidth',3,'marker','+');
    % minimum
   ...
   w_new = w;
   if ECC > 0
     plot(real(w_new), imag(w_new),'color','r','LineWidth',1.2);hold on;
   % initial position
     ...
     plot(real(w_new), imag(w_new),'color','b','LineWidth',1.2);hold on;
   % initial position
     hnd_w=line([0 real(w_new(1))], [0 imag(w_new(1))],'color', [.4 .4 .4], ...
         'linewidth',1.2);  
     ini_w=plot([real(w_new(1))], [imag(w_new(1))],'color', [.4 .4 .4], ...
