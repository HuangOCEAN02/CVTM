...
time = datestr(SDtime);
te = size(SDtime,2); % Length of data 
%% GRID INFO

lono  = double(ncread('../GRID_INFO.nc','var110'));           
lato  = double(ncread('../GRID_INFO.nc','var120'));
masko = double(ncread('../GRID_INFO.nc','var300',[1 1],[ie*je 1 ])); 

lono = reshape(lono,[ie,je]);
lato = reshape(lato,[ie,je]);
masko = reshape(masko,[ie,je]);
...
% tee = length([97:te]); % New length of data to be analyzed
% from 1rd Aug to 30st Sep
tee = length([745:te]);
ts = 745;
% from 1st Sep to 30th Sep
% tee = length([1489:te]);
% ts = 1489;
% from 15th Aug to 30th Sep
%tee = length([1081:te]);
eta_time = zeros(ie,je,tee);
...
for t= ts:1:te
    if (LDTRUN(t)<10)
        name = [Location,'zoo/ZOO_0000000',num2str(LDTRUN(t)),'.nc'];
     else if(LDTRUN(t) < 100)
        name = [Location,'zoo/ZOO_000000',num2str(LDTRUN(t)),'.nc'];
       else if(LDTRUN(t) < 1000)
           name = [Location,'zoo/ZOO_00000',num2str(LDTRUN(t)),'.nc'];
        else if(LDTRUN(t) < 10000)
           name = [Location,'zoo/ZOO_0000',num2str(LDTRUN(t)),'.nc'];
          else if(LDTRUN(t) < 100000)
           name = [Location,'zoo/ZOO_000',num2str(LDTRUN(t)),'.nc'];
          else if(LDTRUN(t) < 1000000)
           name = [Location,'zoo/ZOO_00',num2str(LDTRUN(t)),'.nc'];
          end
          end
        end
      end
    ...
    eta_time(:,:,t-ts+1) = double(squeeze(ncread(name,'ZOO')));
end

% Extract the interior
eta_time([1:spongew],:,:)=[];
eta_time(:,[1:spongen,end-sponges+1:end],:)=[];
...
eta = reshape(eta,[ie*je,tee]);
lono = reshape(lono,[ie*je,1]);
lato = reshape(lato,[ie*je,1]);
masko = reshape(masko,[ie*je,1]);
eta_tide = squeeze(eta);

%% ANALYSIS
disp(['Length of the time seires: ',num2str(size(eta_tide,2))])
disp(['Numbers of the model grid: ',num2str(size(eta_tide,1))])
if ttide == 1
...
%   t_order = [6,8,15,17,5,nan,14,nan];  %the ID, the 35 tide    
    ...
    % from outside loop.
    amph=zeros([ie*je 5]);gphh=zeros([ie*je 5]);res=zeros([4 5]);
parfor i = 1:(ie*je)
    s = eta_tide(i,:);
    ...
    disp(['the current processing Num of eta: ',num2str(i)]); 
end
save('res_eta_level1_CV.mat','-v7.3','amph','gphh','lono','lato','masko','tide_names','t_order');
clearvars -except eta lono lato masko
