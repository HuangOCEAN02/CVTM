...
SG_Ttide = zeros(length(time),8);
SG_Ttide(:,1)=t_predic(time,'M2 ',1/TidePeriod.m2, TIDECON(1,:));
SG_Ttide(:,2)=t_predic(time,'S2 ',1/TidePeriod.s2, TIDECON(2,:));
SG_Ttide(:,3)=t_predic(time,'N2 ',1/TidePeriod.n2, TIDECON(3,:));
SG_Ttide(:,4)=t_predic(time,'K2 ',1/TidePeriod.k2, TIDECON(4,:));
SG_Ttide(:,5)=t_predic(time,'K1 ',1/TidePeriod.k1, TIDECON(5,:));
SG_Ttide(:,6)=t_predic(time,'O1 ',1/TidePeriod.o1, TIDECON(6,:));
SG_Ttide(:,7)=t_predic(time,'P1 ',1/TidePeriod.p1, TIDECON(7,:));
SG_Ttide(:,8)=t_predic(time,'Q1 ',1/TidePeriod.q1, TIDECON(8,:));

SG_Ttide_sum = sum(SG_Ttide,2);
data{3} = tide_M2{14,3};
data{2} = tide_M2{14,2};
%% comparision
...
plot((time-time(1))*24,SG_Ttide_sum*1e-2,'r','linewidth',1.2);hold on
...
% load('../A.2.3(ControlRun)/exp_A22_Senegal.mat')
% plot([1:tee],exp_A22,'g','LineWidth',1.0);hold on

load('Tide_Senegal.mat')
plot([1:tee],exp_SG,'b','LineWidth',1.0);hold on

% from Aug 1st
Mdata = exp_SG(744+1:end);
Odata = SG_Ttide_sum(744+1:end)*1e-2;
Edata = Mdata - Odata;
...
set(gca,'Layer','top');
title(['Dakar Senegal','(',num2str(abs(Lon)),'°W ',num2str(Lat),'°N)'],...
...
img=getframe(gcf);
imwrite(img.cdata,['SG_SSH.tiff'], 'tiff', 'Resolution', 300)
%%
...
save Tide_Senegal.mat  SG_Ttide_sum SG_Ttide time TIDECON Skill Mse Ostd Estd -append
%% Amplitude (m)

SG_selct = exp_SG(97:end);
start_time = [2011,7,05,0,0,0];
[NAME_exp,FREQ_exp,TIDECON_exp,XOUT_exp] = t_tide(SG_selct,'start time',start_time,'latitude',Lat,...
            'interval',1);
...
img=getframe(gcf);
imwrite(img.cdata,['SG_SSH_Amplitude.tiff'], 'tiff', 'Resolution', 300)
...
Phase_select([2]) = Phase_select([2]) - 360;
...
img=getframe(gcf);
imwrite(img.cdata,['SG_SSH_Phase.tiff'], 'tiff', 'Resolution', 300)
