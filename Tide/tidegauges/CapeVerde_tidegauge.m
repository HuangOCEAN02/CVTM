...
% m_proj('miller','lat',LATLIMS,'lon',LONLIMS);
...
CV_Ttide = zeros(length(time),8);
CV_Ttide(:,1)=t_predic(time,'M2 ',1/TidePeriod.m2, TIDECON(1,:));
CV_Ttide(:,2)=t_predic(time,'S2 ',1/TidePeriod.s2, TIDECON(2,:));
CV_Ttide(:,3)=t_predic(time,'N2 ',1/TidePeriod.n2, TIDECON(3,:));
CV_Ttide(:,4)=t_predic(time,'K2 ',1/TidePeriod.k2, TIDECON(4,:));
CV_Ttide(:,5)=t_predic(time,'K1 ',1/TidePeriod.k1, TIDECON(5,:));
CV_Ttide(:,6)=t_predic(time,'O1 ',1/TidePeriod.o1, TIDECON(6,:));
CV_Ttide(:,7)=t_predic(time,'P1 ',1/TidePeriod.p1, TIDECON(7,:));
CV_Ttide(:,8)=t_predic(time,'Q1 ',1/TidePeriod.q1, TIDECON(8,:));
%% comparision
CV_Ttide_sum = sum(CV_Ttide,2);
...
plot((time-time(1))*24,CV_Ttide_sum*1e-2,'r','linewidth',1.2);hold on
...
plot([1:tee],exp_CV,'b','LineWidth',1.0);hold on
Mdata = exp_CV(744+1:end);
Odata = CV_Ttide_sum(744+1:end)*1e-2;
Edata = Mdata-Odata;
...
axis([744+1 1488+1 -0.6 0.8])
set(gca,'fontsize',12,'XTick',[744+1:24:1488+1],'Ytick',[-0.8:0.2:0.8]);
...
set(gca,'Layer','top');
title(['Palmeira, Cape Verde ','(',num2str(abs(Lon)),'°W ',num2str(abs(Lat)),'°N)'],...
...
% img=getframe(gcf);
% imwrite(img.cdata,['CV_SSH.tiff'], 'tiff', 'Resolution', 300)
%%
...
% img=getframe(gcf);
% imwrite(img.cdata,['CV_SSH_LinearFit.tiff'], 'tiff', 'Resolution', 300)

save Tide_CapeVerde.mat  CV_Ttide_sum CV_Ttide time TIDECON Skill Mse Ostd Estd -append
%% Amplitude (m)
CV_selct = exp_CV(97:end);
start_time = [2011,7,05,0,0,0];
[NAME_exp,FREQ_exp,TIDECON_exp,XOUT_exp] = t_tide(CV_selct,'start time',start_time,'latitude',Lat,...
            'interval',1);
% The first main eight exluded P1 K2
% M2 15
% S2 17
% N2 14
% K2
...
text (0.75,0.332,'Observation','Color',[0 0 1],...
    'FontWeight','bold',...
    'FontSize',18,...
    'FontName','Arial');hold on
text (0.75,0.3097,'     Model  ','Color','r',...
    'FontWeight','bold',...
    'FontSize',18,...
    'FontName','Arial');hold on
...
img=getframe(gcf);
imwrite(img.cdata,['CV_SSH_Amplitude.tiff'], 'tiff', 'Resolution', 300)
...
img=getframe(gcf);
imwrite(img.cdata,['CV_SSH_Phase.tiff'], 'tiff', 'Resolution', 300)
