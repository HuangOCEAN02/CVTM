function [h,varargout]=m_ellipse_modify(long,lat,fmaj,fmin,finc,fpha,scl,tpe,varargin)
...
if strcmp(tpe,'line')
  ...
  if pha_flag==1
    ...
    if any(ii)
      hp(ii)=line([X(ii)';xe(1,ii)],[Y(ii)';ye(1,ii)],'linewi',1.5,'color','r');
      hp2(ii)=line([X(ii)';xe(2,ii)],[Y(ii)';ye(2,ii)],'linewi',1.5,'color','r','linest',':');
    end
    if any(~ii)
       hp(~ii)=line([X(~ii)';xe(1,~ii)],[Y(~ii)';ye(1,~ii)],'linewi',1.5,'color','b');
       hp2(~ii)=line([X(~ii)';xe(2,~ii)],[Y(~ii)';ye(2,~ii)],'linewi',1.5,'color','b','linest',':'); 
    end

    scale_bar = find(fmin(:)==0.002);
    hp_bar(scale_bar)=line([X(scale_bar)';xe(1,scale_bar)],[Y(scale_bar)';ye(1,scale_bar)],'linewi',2,'color','k');
    hp_bar2(scale_bar)=line([X(scale_bar)';xe(2,scale_bar)],[Y(scale_bar)';ye(2,scale_bar)],'linewi',2,'color','k','linest',':');
    
    h=[h,hp,hp2,hp_bar,hp_bar2];
